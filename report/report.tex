...
Core responsibilities include \texttt{submit(run, recipe, target\_endpoint)} which generates and submits benchmark jobs, \texttt{stop(job\_id)} which cancels running benchmarks using \texttt{scancel}, \texttt{status(job\_id)} which queries SLURM for job state, and \texttt{\_build\_workload\_command()} which constructs the Python invocation for the workload runner with all necessary parameters. The orchestrator also handles payload generation from dataset specifications, supporting both synthetic datasets defined inline and custom datasets loaded from files.
...
Key methods include \texttt{start\_monitor(recipe\_name, target\_job\_ids, metadata)} which deploys a Prometheus instance with automatic target resolution, \texttt{stop\_monitor(monitor\_id)} which terminates a monitoring stack and cancels all component jobs, \texttt{list\_running\_monitors()} which returns currently active monitors, \texttt{export\_metrics(monitor\_id, format, export\_type)} which exports collected metrics to JSON or CSV formats, and \texttt{\_resolve\_targets(recipe, targets, job\_ids)} which converts service names to actual host:port endpoints using discovery data.
...
The recipe implements validation logic ensuring target specifications are complete and Prometheus configuration is valid. It provides the \texttt{to\_prometheus\_config(resolved\_targets)} method which generates Prometheus YAML configuration from recipe specifications and resolved endpoint mappings.
